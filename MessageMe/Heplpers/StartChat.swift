...
func restartChat(chatRoomId: String, memberIds: [String]){
    ...
    FirebaseUserListener.shared.downlaodUsersFromFirebase(withIDs: memberIds) { (users) in
        if users.count > 0{
...
func createRecentItems(chatRoomId: String, users: [User]) {
    ...
    FirebaseReference(.Recent).whereField(kCHATROOMID, isEqualTo: chatRoomId).getDocuments { (snapshot, error) in
        ...
        for userId in memberIdsToCreateRecent {
            ...
            let recentObject = RecentChat(id: UUID().uuidString, chatRoomId: chatRoomId, senderId: senderUser.id, senderName: senderUser.userName, receiverId: receiverUser.id, receiverName: receiverUser.userName, date: Date(), memberIds: [senderUser.id, receiverUser.id], lastMessage: "", unreadCount: 0, avatarLink: receiverUser.avatarLink)
